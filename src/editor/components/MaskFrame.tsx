...
export const MaskFrame: React.FC = () => {
    ...
    return (
        <section className="rounded-lg border border-[color:var(--border-subtle)] bg-white/5 p-3">
            <h3 className="text-[10px] uppercase tracking-widest text-[color:var(--ui-text)] mb-2">Mask Frames</h3>
            <div className="grid grid-cols-2 gap-2">
                {actions.map((action) => (
                    <button
                        key={action.label}
                        onClick={action.onClick}
                        className="flex items-center gap-2 rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-[11px] uppercase tracking-widest text-[color:var(--ui-text)] hover:bg-white/10 transition-all"
                    >
                        {action.icon}
