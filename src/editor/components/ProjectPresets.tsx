    ...
    <button
      ...
      className={`project-presets-card w-full text-left px-3 py-3 rounded-2xl border transition-all duration-300 ease-in-out flex flex-col gap-1 ${
        preset.name === 'US Letter'
          ? 'project-presets-card-recommended bg-[color:var(--brand-primary)]/20 border-[color:var(--brand-primary)]/50 hover:bg-[color:var(--brand-primary)]/30'
          : 'bg-white/5 border-white/10 hover:bg-white/10'
      } ${!presetsReady ? 'opacity-50 cursor-not-allowed' : ''}`}
    >
      <span className={`project-presets-card-name text-xs uppercase tracking-widest ${preset.name === 'US Letter' ? 'text-[color:var(--brand-primary)]' : 'text-[color:var(--ui-text)]'}`}>{preset.name}</span>
      <span className="project-presets-card-description text-[10px] uppercase tracking-widest text-[color:var(--ui-panel-text)]">{preset.description}</span>
    </button>
  );

  return (
    <section className="project-presets-content px-4 space-y-3">
      <div className="project-presets-intro flex items-center justify-between">
        <h3 className="text-[11px] uppercase tracking-widest text-[color:var(--ui-text)]">New Canvas</h3>
        <span className="text-[9px] uppercase tracking-widest text-[color:var(--ui-panel-text)]">Presets</span>
      </div>
      <div className="project-presets-section space-y-2">
        <div className="project-presets-section-header flex items-center justify-between">
        ...
        <div className="project-presets-card-list space-y-2">
