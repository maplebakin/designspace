    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
      <div className="bg-[color:var(--ui-panel)] rounded-lg shadow-2xl w-full max-w-3xl max-h-[85vh] flex flex-col border border-[color:var(--ui-border)] backdrop-blur-[var(--ui-blur)] text-[color:var(--ui-text)]">
        <header className="flex items-center justify-between p-4 border-b border-[color:var(--ui-border)]">
          <h2 className="text-[11px] uppercase tracking-widest text-slate-200">Vibe Settings</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
          <section className="space-y-4">
            <h3 className="text-sm uppercase tracking-widest text-slate-300">Import UI Theme</h3>
            <input
              type="file"
              ref={fileInputRef}
          ...
          <section className="space-y-4">
            <h3 className="text-sm uppercase tracking-widest text-slate-300">Preset Vibes</h3>
            <div className="grid grid-cols-3 gap-3">
              {UI_THEME_PRESETS.map((preset) => (
                ...
                >
                  ...
                  <div className="text-xs uppercase tracking-widest text-slate-200">{preset.name}</div>
                  <div className="text-[10px] uppercase tracking-widest text-slate-400">{preset.description}</div>
                </button>
              ))}
            </div>
          </section>
