...
export const ExportModal: React.FC<ExportModalProps> = ({ isOpen, onClose, format }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
      <div className="bg-[color:var(--ui-panel)] rounded-lg shadow-2xl w-full max-w-sm border border-[color:var(--ui-border)] backdrop-blur-[var(--ui-blur)] text-slate-100">
        <header className="flex items-center justify-between p-4 border-b border-[color:var(--border-subtle)]">
          <h2 className="text-[11px] uppercase tracking-widest text-slate-100">Export as {format.toUpperCase()}</h2>
        ...
        <div className="p-6 space-y-4">
            {format === 'jpeg' && (
                <div className='space-y-2'>
                    <label className="text-[10px] uppercase tracking-widest text-slate-200">Quality ({quality}%)</label>
                    <input
                        type="range"
                        min="1"
                        max="100"
